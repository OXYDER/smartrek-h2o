...
export function NewSensorModal({ siteId, onClose, onCreated }: Props) {
  ...
  const [kind, setKind] = useState<ChannelKind>('temperature')
  ...
  async function handleCreate() {
    ...
    const sensor = await smartrekClient.createSensor({
      name: name.trim(),
      siteId,
      initialChannel: { label: CHANNEL_KIND_LABELS[kind], kind, unit: CHANNEL_KIND_UNITS[kind] },
    })
    setCreating(false)
    onCreated(sensor)
  }
      ...
      <div className="relative w-full max-w-sm bg-panel border border-line rounded-lg p-5 flex flex-col gap-4">
        <div>
          ...
          <p className="text-xs text-muted mt-0.5">
            Tu pourras ajouter d'autres canaux ensuite dans le détail du capteur.
          </p>
        </div>
        ...
        <label className="flex flex-col gap-1 text-sm">
          <span className="text-muted text-xs font-mono uppercase">Premier canal</span>
          <select
            value={kind}
